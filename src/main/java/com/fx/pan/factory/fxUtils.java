...
import com.fx.pan.factory.exception.fxException;
...
public class fxUtils {
    ...
    public static int getFileTypeByExtendName(String extendName){
        ...
        if (Arrays.asList(IMG_FILE).contains(extendName)){
            ...
            fileType = 4;
        }else if (Arrays.asList(MUSIC_FILE).contains(extendName)){
            fileType = 5;
    ...
    public static String getUploadFileUrl(String identifier, String extendName) {
        ...
        if (!dir.exists()) {
            ...
            if (!result) {
                throw new fxException("创建upload目录失败：目录路径：" + dir.getPath());
            }
        }
